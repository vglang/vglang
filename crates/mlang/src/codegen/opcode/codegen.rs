...
use crate::codegen::{
    opcode::{
        ...
        gen_opcode_definition, gen_sexpr_mod, gen_variable_mod, SexprDefault, SexprInit,
        TypeDefinition,
    },
...
impl CodeGen for OpcodeCodeGen {
    ...
    fn gen(self) -> TokenStream {
        ...
        let sexpr_init_fns = self
            .node_gens
            .iter()
            .map(|node| node.gen_sexpr_init_fns())
            .chain(self.enum_gens.iter().map(|node| node.gen_sexpr_init_fns()))
            .collect::<Vec<_>>();

        let sexpr_default_fns = self
            .node_gens
            .iter()
            .map(|node| node.gen_sexpr_default_fns())
            .chain(
                self.enum_gens
                    .iter()
                    .map(|node| node.gen_sexpr_default_fns()),
        ...
        let sexpr_from_fns = self
            .node_gens
            .iter()
            .map(|node| node.gen_sexpr_from_fns())
            .chain(self.enum_gens.iter().map(|node| node.gen_sexpr_from_fns()))
            .collect::<Vec<_>>();
