...
use crate::lang::{
    ...
    rustgen::mapping::{ComplexTypeMapping, FieldMapping, IdentMapping, TypeMapping},
};

trait SexprElementCodeGen {
    fn gen_el_fns(&self, ty_mod: &TokenStream) -> TokenStream;
}

trait SexprLeafCodeGen {
    fn gen_leaf_fns(&self, ty_mod: &TokenStream) -> TokenStream;
}

trait SexprAttrCodeGen {
    fn gen_attr_fns(&self, ty_mod: &TokenStream) -> TokenStream;
}

trait SexprDataCodeGen {
    fn gen_data_fns(&self, ty_mod: &TokenStream) -> TokenStream;
}

trait SexprRestraintCodeGen {
    fn gen_restraint_trai_impls(&self, ty_mod: &TokenStream) -> TokenStream;
}

impl SexprElementCodeGen for Node {
    fn gen_el_fns(&self, opcode_mod: &TokenStream) -> TokenStream {
        let mut stats = vec![];
...
impl SexprLeafCodeGen for Node {
    fn gen_leaf_fns(&self, opcode_mod: &TokenStream) -> TokenStream {
        let mut stats = vec![];
        stats.push(self.gen_from_fn(opcode_mod));
        stats.push(self.gen_default_fn(opcode_mod));
        stats.push(self.gen_chain_init_fns(opcode_mod));
...
impl SexprAttrCodeGen for Node {
    fn gen_attr_fns(&self, opcode_mod: &TokenStream) -> TokenStream {
        let mut stats = vec![];
        stats.push(self.gen_from_fn(opcode_mod));
        stats.push(self.gen_default_fn(opcode_mod));
        stats.push(self.gen_chain_init_fns(opcode_mod));
...
impl SexprRestraintCodeGen for ApplyTo {
    fn gen_restraint_trai_impls(&self, opcode_mod: &TokenStream) -> TokenStream {
        let mut stats = vec![];

        for from in &self.from {
            let from = from.to_type_name();
            for to in &self.to {
                let to = to.to_type_name();
                stats.push(quote! {
...
impl SexprRestraintCodeGen for ChildrenOf {
    fn gen_restraint_trai_impls(&self, opcode_mod: &TokenStream) -> TokenStream {
        let mut stats = vec![];
...
impl Node {
    fn gen_from_fn(&self, opcode_mod: &TokenStream) -> TokenStream {
        ...
        for field in self.fields.iter() {
            ...
            let from_expr = field.to_from_clause(&quote! {}, &value);

            stats.push(field.to_init_clause(&from_expr));

            if let Some(ident) = field.ident() {
                generics.push(ident.to_type_name());
            } else {
                generics.push(format!("P{}", tuple_index).parse().unwrap());
            }

            where_clauses.push(field.ty().to_from_where_clause(
                opcode_mod,
                &quote! {},
        ...
        let body = self.to_struct_body(stats);
    ...
    fn gen_default_fn(&self, opcode_mod: &TokenStream) -> TokenStream {
        ...
        let body = self.to_struct_body(init_stats);
    ...
    fn gen_chain_init_fns(&self, opcode_mod: &TokenStream) -> TokenStream {
        ...
        for field in self.fields.iter() {
            ...
            let where_clause = field
                .ty()
                .to_from_where_clause(opcode_mod, &quote! {}, &param);

            let fn_name: TokenStream = field.ident().unwrap().to_field_name();

            let assign = field.to_from_clause(&quote! {}, &quote! { value });
...
impl SexprDataCodeGen for Node {
    fn gen_data_fns(&self, opcode_mod: &TokenStream) -> TokenStream {
        let mut stats = vec![];
        stats.push(self.gen_from_fn(opcode_mod));
        stats.push(self.gen_default_fn(opcode_mod));
...
impl SexprDataCodeGen for Enum {
    fn gen_data_fns(&self, opcode_mod: &TokenStream) -> TokenStream {
        let mut impls = vec![];

        let enum_ident = self.to_ident();
        ...
        for node in self.fields.iter() {
            ...
            let field_ident = node.ident.to_field_name();
            ...
            let node_ident = node.to_ident();

            let body = node.to_struct_body(stats);
...
impl CodeGen {
    ...
    pub fn new(opcode_mod: impl AsRef<str>, tuple_max_len: usize) -> Self {
    ...
    pub fn gen(mut self, stats: &[Stat]) -> TokenStream {
        ...
        for stat in stats {
            match stat {
                Stat::Element(node) => {
                    token_streams.push(node.gen_el_fns(&self.opcode_mod));
                    self.collect_map_collect_types(node);
                }
                Stat::Leaf(node) => {
                    token_streams.push(node.gen_leaf_fns(&self.opcode_mod));
                    self.collect_map_collect_types(node);
                }
                Stat::Attr(node) => {
                    token_streams.push(node.gen_attr_fns(&self.opcode_mod));
                    self.collect_map_collect_types(node);
                }
                Stat::Data(node) => {
                    token_streams.push(node.gen_data_fns(&self.opcode_mod));
                    self.collect_map_collect_types(node);
                }
                Stat::Enum(node) => {
                    token_streams.push(node.gen_data_fns(&self.opcode_mod));
                ...
                Stat::ApplyTo(node) => {
                    token_streams.push(node.gen_restraint_trai_impls(&self.opcode_mod));
                }
        ...
        for field in node.fields.iter() {
            match field.ty() {
                Type::ListOf(component, _) => {
                    let ident = component.to_definition(&quote! {}).to_string();
                    match ident.as_str() {
                        "Point" => {}
                        _ => {
    ...
    fn gen_tuple_impls(&self) -> TokenStream {
        ...
        for (_, ty) in &self.map_collect_types {
            ...
            let where_clause = ty.to_from_where_clause(&self.opcode_mod, &quote! {}, &param_type);

            let param = quote! {self};

            let field = ty.to_from_clause(&quote! {}, &param);
    ...
    fn gen_tuple_map_collect_impl(&self, len: usize) -> TokenStream {
        ...
        for (_, ty) in &self.map_collect_types {
            ...
            for i in 0..len {
                ...
                where_clauses.push(ty.to_from_where_clause(
                    &self.opcode_mod,
                    &quote! {},
                    &param_type,
                ));
                ...
                fields.push(ty.to_from_clause(&quote! {}, &param));
...
pub fn gen_sexpr_mod(
    stats: impl AsRef<[Stat]>,
    opcode_mod: impl AsRef<str>,
    tuple_max_len: usize,
) -> TokenStream {
    CodeGen::new(opcode_mod, tuple_max_len).gen(stats.as_ref())
}
