...
pub fn gen<P: AsRef<std::path::Path>>(opcodes: &[crate::ir::Stat], target_dir: P) {
    ...
    let binary_mod = binary::BinaryModGen::new("super::opcode::").gen(opcodes);
    ...
    let binary_file = target_dir.as_ref().join("binary.rs");
    ...
    write_and_fmt_rs("binary.rs", binary_file, binary_mod.to_string());
